...
from app.services.image_overlay import image_overlay_service
...
class PublishService:
    ...
    def publish_content(self, content_id: str, payload: ContentPublishRequest, context: RequestContext) -> ContentPublishResponse:
        ...
        if not content or content["status"] == ContentStatus.DELETED:
        ...
        selected_variant_asset_id = payload.selected_variant_asset_id or content.get("selected_variant_asset_id")
        image_variant_job_id = content.get("image_variant_job_id")
        image_variant_provider = None
        publish_thumbnail = None
        external_post = {
        ...
        if payload.apply_image_variant:
            ...
            if payload.image_variant_provider == ImageVariantProvider.NANO_BANANA:
                ...
                if not content.get("variant_asset_ids"):
                    ...
                    content["variant_asset_ids"] = variant_asset_ids
                    if not selected_variant_asset_id and variant_asset_ids:
                        selected_variant_asset_id = variant_asset_ids[0]
                elif not image_variant_job_id:
                    image_variant_job_id = f"job_image_{uuid4().hex[:8]}"

        publish_thumbnail = image_overlay_service.render_publish_thumbnail(
            merchant_id=content["merchant_id"],
            content=content,
            selected_variant_asset_id=selected_variant_asset_id,
        ...
        db_repository.create_publish_result({
            ...
            "thumbnail_url": publish_thumbnail["preview_url"] if publish_thumbnail else None,
            "title": content["title"],
        ...
        audit_service.record(
            ...
            metadata={
                ...
                "thumbnail_asset_id": publish_thumbnail["asset_id"] if publish_thumbnail else "",
            },
        )
